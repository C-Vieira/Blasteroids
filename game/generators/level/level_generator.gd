...
var crystal_scene: PackedScene = preload("res://entities/crystal/crystal.tscn")
	...
	for x: int in range(map.position.x, map.end.x):
		for y: int in range(map.position.y, map.end.y):
			...
			if cell == Vector2i(1, 0): #Spawn Regolith tiles on ground
				var new_tile: RegolithTileEntity = regolith_tile.instantiate()
...
func _on_regolith_tile_destroyed(pos: Vector2, had_crystals: bool) -> void:
	if had_crystals:
		spawn_crystal(pos)

func spawn_crystal(pos: Vector2) -> void:
	var crystal: Node2D = crystal_scene.instantiate()
